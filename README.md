...
## การใช้งานจริง (อัปเดตสำคัญ)

`index.html` (หน้าเว็บแอป) ถูก serve จาก backend เดียวกันนี้โดยตรง (`express.static`) — เปิด URL ของ
Render ตรงๆ (เช่น `https://rain-alert-backend-xxxx.onrender.com`) ก็จะได้หน้าแอปพร้อมข้อมูลฝนจริงทันที
ไม่ต้องพึ่ง Claude artifact อีกต่อไป เพราะหน้าเว็บที่ publish ผ่าน claude.ai ไม่สามารถยิง fetch ไปยัง
เซิร์ฟเวอร์ภายนอกได้ (ข้อจำกัดด้านความปลอดภัยของแพลตฟอร์ม) — serve ทั้งคู่จาก origin เดียวกันแบบนี้จึง
เป็นทางที่ใช้ได้จริง แนะนำให้บันทึก/เพิ่มไปยังหน้าจอโฮมจาก URL ของ Render โดยตรงแทนลิงก์ artifact เดิม
